<div class="bg-white px-32 pt-16 pb-10">
    <div class="grid grid-cols-3">
        <div class="col-span-2">
            <h6 class="font-semibold text-xl">Saung Komando Ciwidey</h6>
            <p class="text-[#808080] my-5">Lebakmuncang, Kec. Ciwidey, Kab. Bandung, <br />Jawa Barat 40973</p>
            <div class="flex gap-3">
                <div class="bg-black rounded-full h-10 w-10 flex justify-center items-center">
                    <img src="{{ asset('assets/icon/tiktok.svg') }}" alt="Tiktok">
                </div>
                <div class="bg-black rounded-full h-10 w-10 flex justify-center items-center">
                    <img src="{{ asset('assets/icon/whatsapp.svg') }}" alt="Whatsapp" class="w-6 h-6">
                </div>
                <div class="bg-black rounded-full h-10 w-10 flex justify-center items-center">
                    <img src="{{ asset('assets/icon/instagram.svg') }}" alt="Instagram" class="w-9 h-9">
                </div>
            </div>
        </div>
        <div class="grid grid-cols-3">
            <div>
                <h6 class="font-semibold text-xl">Menu</h6>
                <ul class="mt-5 text-[#808080] space-y-1">
                    <li>Beranda</li>
                    <li>Fasilitas</li>
                    <li>Geleri</li>
                    <li>Kontak</li>
                    <li>Google Maps</li>
                </ul>
            </div>
            <div>
                <h6 class="font-semibold text-xl">Fasilitas</h6>
                <ul class="mt-5 text-[#808080] space-y-1">
                    <li>Penginapan</li>
                    <li>Kolam Renang</li>
                    <li>Gym Area</li>
                    <li>Mini Soccer</li>
                    <li>Offroad</li>
                </ul>
            </div>
            <div>
                <ul class="mt-12 text-[#808080] space-y-1">
                    <li>Graha Aula</li>
                    <li>Rumput Sitetis</li>
                    <li>Mini Zoo Aviary</li>
                    <li>Camping Ground</li>
                    <li class="font-semibold">Lihat Semua Fasilitas</li>
                </ul>
            </div>
        </div>
    </div>
    <div class="flex justify-between">
        <p class="text-[#000000] mt-4 font-semibold">© 2025 Saung Komando Ciwidey. Hak Cipta Dilindungi.</p>
        <div class="flex gap-10">
            <p class="text-[#000000] mt-4 font-semibold">Syarat dan ketentuan</p>
            <p class="text-[#000000] mt-4 font-semibold">Kebijakan Privasi</p>
        </div>
    </div>
</div>